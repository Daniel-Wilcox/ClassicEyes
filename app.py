...
WATI_TEMPLATE_DICT = {
    cls.__name__.removesuffix("Handler"): cls for cls in ALL_WATI_HANDLERS_LIST
}

CE_PRACTICE_LIST = ["Pavilion", "La Lucia"]
...
class HomePage(AbstractPage):
    ...
    def __init__(self, parent, controller):
        ...
        self.practice_label_default = tk.StringVar(
            value="Select Classic Eyes Practice: "
        )
    ...
    def load_widgets(self):
        ...
        self.template_menu = tk.OptionMenu(
            self, self.template_default, *self.template_list
        )
        ...
        self.practice_menu = tk.OptionMenu(
            self, self.practice_default, *CE_PRACTICE_LIST
        )
        ...
        self.upload_button = tk.Button(
            self, text="Select File", command=self._update_uploaded_filename
        )
        ...
        self.process_button = tk.Button(
            self, text="Process File", command=self._validate_and_process
        )
    ...
    def _validate_and_process(self):
        ...
        if not selected_template:
            messagebox.showwarning(
                "Variable Missing", "Please select Wati Template before processing."
            )
            return
        ...
        if not selected_practice:
            messagebox.showwarning(
                "Variable Missing",
                "Please select Classic Eyes Practice before processing.",
            )
            return
        ...
        if not selected_filepath:
            messagebox.showwarning(
                "File Missing",
                "Please upload a TOMs Excel Report file before processing.",
            )
            return
    ...
    def _process_file_with_handler(
        self, process_handler: AbstractHandler, file_path: str
    ):
...
class LoadingPage(AbstractPage):
    ...
    def load_widgets(self):
        ...
        self.cancel_button = tk.Button(
            self, text="Cancel", command=self._cancel_process
        )
...
class DownloadPage(AbstractPage):
    ...
    def load_widgets(self):
        ...
        self.download_button_2 = tk.Button(
            self,
            text="Auto Save",
            command=lambda: self._download_results(ask_user=False),
        )
        self.download_button_2.pack(pady=10)

        self.generate_another_button = tk.Button(
            self, text="Generate Another", command=self._generate_another
        )
    ...
    def _download_results(self, ask_user: bool = False):
        ...
        messagebox.showinfo(
            "Download", f"File {show_save_name} successfully saved at {show_save}."
        )
